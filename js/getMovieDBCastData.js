function getMovieDBCastData (id) {
	var xmlURL = '', xmlURL2 = '';
	xmlURL = 'json/moviedb/cast/' + id + '-cast.json'; dataType = 'json';
	...
	$.each(result.cast, function(i, row) {
		...
		if (totalCastCount < 6) {
			...
			if (row.profile_path != null) {
				...
				castContainer += '<div><img align=\"left\" class=\"castPhoto reflectBelow2\" src=\"http://neo.mayo.edu/cdh/modProject/images/movies/cast/' + id + row.profile_path + '\" onerror=\"imageError(this, \'cast\',\'' + row.profile_path + '\');\" /></div>';
			} else {
